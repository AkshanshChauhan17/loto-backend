    ...
    try {
        const resultData = await tx(async(conn) => {
            ...
            const [resultInsert] = await conn.execute(
                `INSERT INTO results (game_id, winning_numbers, bonus) VALUES (?, ?, ?)`, [game_id, JSON.stringify(winningNumbers), bonus || null]
            );
            ...
            const [tickets] = await conn.execute(
                `SELECT * FROM tickets WHERE game_id = ? AND status = 'PENDING'`, [game_id]
            );
            ...
            for (const ticket of tickets) {
                ...
                const [lines] = await conn.execute(
                    `SELECT * FROM ticket_lines WHERE ticket_id = ? AND status = 'PENDING'`, [ticket.id]
                );
                ...
                for (const line of lines) {
                    ...
                    if (payout > 0) {
                        await conn.execute(
                            `UPDATE ticket_lines SET status = 'WIN', win_amount = ? WHERE id = ?`, [payout, line.id]
                        );
                        ticketWinTotal += payout;
                    } else {
                        await conn.execute(
                            `UPDATE ticket_lines SET status = 'LOSE', win_amount = 0 WHERE id = ?`, [line.id]
                        );
                    }
                }
                ...
                if (ticketWinTotal > 0) {
                    await conn.execute(
                        `UPDATE tickets SET status = 'WON' WHERE id = ?`, [ticket.id]
                    );
                    winners.push({
                        ticket_id: ticket.id,
                    ...
                    await conn.execute(
                        `UPDATE tickets SET status = 'LOST' WHERE id = ?`, [ticket.id]
                    );
                }
            }
